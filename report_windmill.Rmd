...
In the following code, I created a new dataset (`elt_w1_clean`) that selects out all but one of the metadata variables, `response_id`. This variable is an unique identifier assigned by Qualtrics that resulted handy in dealing with duplicated ids. 
...
In the code below, the function [pivot_longer](https://tidyr.tidyverse.org/articles/pivot.html) makes the dataset "long" as it increases the number of rows and decreases the number of columns. This function gathers variables' names under the new variable `item_2` and gathers the values of these variables under the new variable `spanish_comfort`. Then, I chose only the options that had a value of 1, indicating when a participant chose that response option. 
...
In the code below, I used the [rename](https://www.rdocumentation.org/packages/reshape/versions/0.8.8/topics/rename) function to rename some of the demographic variables that I will use to describe participants's characteristics in the next section of this report. This function uses a "new name" = "old name" pattern. Very straightforward! 
...
In the code below I applied a function I created `recode_missing_df` (code of the function not shown) to recode these values as `NA`, the way R codes missing values. 
...
In this section, I describe the scale creation process for major strudy constructs (i.e. school climate, adjustment, and quality of relationships between participants). 

## Educator's scales {-}

The first step in the scale creation process was to identify which items needed to be reverse coded. Unfortunately, there was no indication in the original survey of which items needed to be reverse coded, so I had to use my subjective judgement to do identify them. 

This was a time consuming process because it entailed an item by item and then a scale by scale review. Luckily, once the items were identified, the reverse coding process was really fast because [likert_reverse](https://rdrr.io/github/martinctc/surveytoolbox/man/likert_reverse.html) does the "heavy lifting" for you! Note that I use this function within a mutate function call:
...
The following are the items I reverse coded: 
