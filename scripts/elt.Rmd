...
spa_2 <- spa %>% 
  distinct(pj, .keep_all = TRUE) # Option 1, not at all comfortable was chosen because was first option.  
...
# joining dataframes with language variables
...
duplicated(elt_w1_clean_4$pj) # no duplicates
...
duplicated(elt_w1_clean_4$pj)

```

```{r}
# meaningful renaming of vars
...
test <- elt_w1_clean_4 %>%
  ...
  select(-q127, -q129, -q131_3_text)

# deleted: q127, q129, q131_3_text because they did not have meaningful info

# next step: export to SPSS to see how it looks and if labels get exported.
```
