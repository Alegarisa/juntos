...
# Number of participants in Wave 2 (taking out duped cases 405, 408, 605)
```{r}

test_set <- p_w3_clean_3 %>%
  filter(response_id != "R_cXQAULkCQKs9ZWd") %>% 
  filter(response_id != "R_eWNINjAZviHsnzv") %>% 
  filter(response_id != "R_0jEpFqwcLyP4yDX") 
...
unique(test_set$family_id) # 86 unique id values, thus 86 families in W2. 

data.frame(table(test_set$participant_id)) # 21 duplicated ids that match two-parent families in w1, 1 family (id 415) three-parent fam. 
```