...
p_w1_clean <- p_w1_raw %>% 
  ...
  filter(id != "999") %>% # deleting test rows
  filter(response_id != "R_31L6rXjsdcuBaz0") %>% # deleting duplicated row with incomplete data (id 312)
...
test <- full_join(p_w1_clean_mom, p_w1_clean_dad) # it maybe worked?
  

test_2 <- full_join(test, p_w1_clean_step_mom) %>%
  select(school_id, condition, response_id, id_mom, id_dad, id_step_par, everything())
...
# exporting to .sav to check proper coding of condition and labels
test_2 %>%
  haven::write_sav(here("nopublish", "test_2.sav"))
```
