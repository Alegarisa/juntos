...
## parent confidence (SELF-EFFICACY)
...
contact_school <- data.frame(p_w1_clean_6[,34:35]) #q18-q19
alpha(contact_school) # alpha = .55 --> poor consistency

# [,39:46] = q23 - q30 youth support? 

par_efforts <- data.frame(p_w1_clean_6[,47:57]) #q31-q41
alpha(par_efforts) # alpha = .87 --> good consistency

par_confid <- data.frame(p_w1_clean_6[,58:62]) #q42-q46
alpha(par_confid) # alpha = .80 --> good consistency

par_school_community <- data.frame(p_w1_clean_6[,63:69]) #q47-q53 
alpha(par_school_community) # alpha = .84 --> good consistency

# par_school_belong <- p_w1_clean_6 %>%
#   select(63:65, 68, 69) %>% 
#   data.frame() # i don't think that 66 = q50 & 67 = q51 go well construct-wise, but alpha is better. What to do? --> keep scale as is. 
#   
# alpha(par_school_belong) # alpha = .79 --> fair consistency  

par_school_trust <- data.frame(p_w1_clean_6[,70:77]) #q54-q61
alpha(par_school_trust) # alpha = .91 --> strong consistency, but I think it may be because the number of items. I believe there is more than one factor in this scale. 

par_endorse <- p_w1_clean_6 %>%
  select(70:73) %>%
  data.frame() # parent endorsement scale from PTIQ scale parent endorsement of school. 
alpha(par_endorse) # alpha = .87 --> good consistency

par_teach_rel <- data.frame(p_w1_clean_6[,78:83]) #q62-q67
...
p_w1_clean_scales <- p_w1_clean_6 %>%
  ...
  mutate(parent_teach_rel = mean(c(q62, q63, q64, q65, q66, q67), na.rm = TRUE),
         parent_effort = mean(c(q31, q32, q33, q34, q35, q36, q37, q39, q40, q41), na.rm = TRUE)) #re-organize these new vars to coincide with numbered order
