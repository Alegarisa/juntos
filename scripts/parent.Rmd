...
#   
#   note: delete this one when having dataset with collapsed and renamed vars
```
...
```{r}
# renaming demo vars english_comfort
...
```{r}
# recode them as N/A with function
...
contact_school <- data.frame(p_w1_clean_5[,34:35])
alpha(contact_school) # alpha = .99

par_teach_rel <- data.frame(p_w1_clean_5[,78:83])
alpha(par_teach_rel) # alpha = .82

p_w1_clean_scales <- p_w1_clean_6 %>%
  rowwise() %>% 
  mutate(parent_teach_rel = mean(c(q62, q63, q64, q65, q66, q67, na.rm = TRUE))) # NAs are not being removed. 

p_w1_clean_scales <- p_w1_clean_6 %>%
  rowwise() %>% 
  mutate(parent_teach_rel = mean(c(q62, q63, q64, q65, q66, q67, digits = 2), na.rm = TRUE)) # NAs are  being removed, but values are rounded to nearest unit. 

# next step: figure this inconsistency out
```
