...
```{r}
p_w1_clean_3 %>%
  filter(family_id == "106")

p_w1_clean_3 %>%
  filter(participant_id == "301_2")# participant dropped from study and is -99 throughout. Id not present in w2 and w3.

p_w1_clean_3 %>%
  filter(family_id == "504")

# no id 106 (es lu), 301 (dropped), 504 (La Mi Ca) in students, but present in parents 
# no id 623 in parents (not even in home visit), but present in students
...
```
next step from 04.01.22: check student wave 1 for ids 106 and 504. 
