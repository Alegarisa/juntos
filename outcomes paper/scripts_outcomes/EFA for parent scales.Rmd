...
# Parallel analysis suggests that the number of factors =  3, multiple warnings
```
...
## Removing crossloading items 35, 40, 41
```{r}
scho_inv <- parent_w1 %>%
...
```
## removing 36 because it doesn't jive as well with other items conceptually
```{r}
scho_inv_fa2 <- parent_w1 %>%
  select(q37, q38, q39)
...
## trying only a couple of items that tap more closely on school endorsemnet
54.	… esta escuela es un buen lugar para mi joven. _____
55.	… el personal de la escuela de mi joven está haciendo cosas buenas por ella / el. _____
56.	… la gente en la escuela de mi joven es confiable. _____
57.	… la escuela de mi joven hace un buen trabajo preparando a los jóvenes para sus futuros. _____
...
factor_test_uls
# no warnings but items 55 with a loading of 1
```
...
cor.plot(poly$rho, numbers = T, upper = F, main = "Polychoric", show.legend = F) # very high corrs - prob multicollinearity
```
...
Conclusion: EFA not possible with this scale. However, the scale was included in multiple regression due to importance to assess parent-teacher re;ationship. Alpha is good. 
