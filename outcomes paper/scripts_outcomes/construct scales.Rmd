...
# 
# alpha(posinvo) # 0.54 poor (0.55 if all caregivers included)
```
...
alpha(posinvo_2) # 0.91 (0.90 if all caregivers included)
```
...
# 
# alpha(skill) # 0.41 (0.32 poor if all caregivers included)
```
...
# Not used because these are not discipline but skill encouragement
134.	Cuando mi joven está aprendiendo un nuevo comportamiento (por ejemplo: ser más responsable, estudioso/a u organizado/a), reconozco su progreso con, por ejemplo, un abrazo, una sonrisa o un pequeño regalo
135.	Cuando mi joven se enfrenta a un gran desafío o establece una meta, le ayudo a centrarse en los pequeños pasos para lograr esa meta.
...
# 
# alpha(discip_2) # 0.83 (0.83 if all caregivers included)
```
...
*Items would be too similar to appropiate discipline. 

```{r}
gen_par <- d2_par_w1 %>%
  select(q128:q136) # NO EFA here, just face value.

alpha(gen_par) # 0.69 (136 is neg corr with tot scale, remowing it would yield 0.74)
```
...
```{r}
navigating <- d2_par_w1 %>%
  select(q31, q32, q33, q34, q35) # may be worth adding q38 and q39 to cover parent social networks (0.83)

alpha(navigating) # 0.86 (0.83 if all caregivers included)
```
...
```{r}
test1 <- d2_par_w1 %>% # same as navigating plus ged item
  select(q31, q32, q33, q34, q35, q36) # no EFA, just face value

alpha(test1) # 0.84
##
test2 <- d2_par_w1 %>% # taps on connecting with parents
  select(q37, q38, q39) # no EFA, just face value

alpha(test2) # 0.79
##
test3 <- d2_par_w1 %>% # taps on knowing rules
  select(q33, q34, q35, q36, q40) # no EFA, just face value
...
fa_2 <- d2_par_w1 %>%
  select(q73, q75, q80, q81)
alpha(fa_2) # 0.77

fa_3 <- d2_par_w1 %>%
  select(q68, q69, q70, q71, q72)
alpha(fa_3) # 0.80
```
