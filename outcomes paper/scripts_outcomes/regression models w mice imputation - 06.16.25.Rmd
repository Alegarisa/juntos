...
d1 <- d %>%
  select(all_of(c(
    ...
    "y_sch_eng", "y_hw", "gen_grades", "grades"
  )))
...
standardize <- function(df) {
  df <- df %>%
    mutate(across(c(selfeffic_1, y_sch_eng, y_hw, gen_grades, grades, parent_eng_comf, youth_age, acadhwi_1, checkhwi_1, struct_1, knowschool_1, stratsbi_1, belong_1, endorse_1, ptrel_1), ~ scale(.)[,1]))  # Standardizing variables
  return(df)
...
These models responded to the changes after working on supplementary materials R markdown. 
...
```{r}
# without support
fit3_st <- with(imputed_data_std, lm(y_hw ~ parent_eng_comf + youth_age + youth_gender + acadhwi_1 + checkhwi_1 + struct_1 + knowschool_1 + stratsbi_1 + belong_1 + endorse_1 + ptrel_1))
...
```{r}
# without support
fit7_st <- with(imputed_data_std, lm(gen_grades ~ parent_eng_comf + youth_age + youth_gender + acadhwi_1 + checkhwi_1 + struct_1 + knowschool_1 + stratsbi_1 + belong_1 + endorse_1 + ptrel_1))
...
```{r}
# without support
fit3_st <- with(imputed_data_std, lm(y_hw ~ parent_eng_comf + youth_age + youth_gender + acadhwi_1 + checkhwi_1 + struct_1 + knowschool_1 + stratsbi_1 + belong_1 + endorse_1))

est3_st <- pool(fit3_st)
summary(est3_st) # no predictors were significantly related with the outcome

pool.r.squared(fit3_st) # 0.12
```
...
```{r}
# without support
fit7_st <- with(imputed_data_std, lm(gen_grades ~ parent_eng_comf + youth_age + youth_gender + acadhwi_1 + checkhwi_1 + struct_1 + knowschool_1 + stratsbi_1 + belong_1 + endorse_1))

est7_st <- pool(fit7_st)
summary(est7_st)

pool.r.squared(fit7_st) # 0.20
```